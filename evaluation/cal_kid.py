...
parser.add_argument("--categories", nargs="+", default=None,
                    help="Categories to evaluate; defaults to folders under generated_path")
...
def calculate_kid(activations_real, activations_generated, subsample_size=None, num_subsets=10):
    ...
    if subsample_size is None:
        subsample_size = min(10, n_real, n_gen)
    else:
        subsample_size = min(subsample_size, n_real, n_gen)
    if subsample_size < 2:
        return None
...
def get_categories_and_classes(folder_path, categories=None):
    if not os.path.isdir(folder_path):
        raise FileNotFoundError(f"Generated result directory does not exist: {folder_path}")
    ...
    for category in categories:
        ...
        if not os.path.isdir(category_path):
            ...
            print(f"[WARN] Directories under generated_path: {available}")
...
def main():
    ...
    category_class_dict = get_categories_and_classes(generated_folder_path, args.categories)
