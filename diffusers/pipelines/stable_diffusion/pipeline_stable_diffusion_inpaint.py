...
class StableDiffusionInpaintPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # ---------------------------------------------------
        # 2) latents 초기화 (init_image -> vae encode or noise)
        # ---------------------------------------------------
        ...
        if latents is None:
            ...
            if strength == 1.0:
                ...
                # e.g. latents = self.scheduler.add_noise(init_image_latents, noise, 첫 번째 timestep)
                # timesteps 설정 전이므로, 일단 아래처럼
                latents = noise  # 뒤에서 scale or etc
        else:
        ...
        # strength 로직 => get_timesteps

        # ---------------------------------------------------
        # 6) extra_step_kwargs + mask_for_anomaly
        # ---------------------------------------------------
        extra_step_kwargs = {}
        scheduler_step_parameters = inspect.signature(
            self.scheduler.step
        ).parameters
        if "eta" in scheduler_step_parameters:
            extra_step_kwargs["eta"] = eta
        if "anomaly_strength" in scheduler_step_parameters:
            extra_step_kwargs["anomaly_strength"] = anomaly_strength
        if "generator" in scheduler_step_parameters:
            extra_step_kwargs["generator"] = generator
        # The custom MAGIC DDIM scheduler accepts the anomaly mask, while the
        # standard Diffusers DDIM scheduler does not. Only forward the keyword
        # when the active scheduler explicitly supports it. This keeps the
        # standard baseline and E1 loss-only inference path compatible.
        if "mask_for_anomaly" in scheduler_step_parameters:
            extra_step_kwargs["mask_for_anomaly"] = mask
