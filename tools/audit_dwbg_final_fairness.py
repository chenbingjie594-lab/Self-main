...
def main():
 ...
 report={'candidate_pool_frozen':f.get('candidate_pool_frozen') is True,'real_train_identical':len(set(real))==1,'validation_identical':len(set(vals))==1,'validation_has_no_synthetic':all(not list((a.dataset_root/g/'images/val').glob('syn_*')) and not list((a.dataset_root/g/'labels/val').glob('syn_*')) for g in groups),'synthetic_counts':syn,'random_and_dwbg_same_frozen_pool':r.get('candidate_pool_frozen') is True and d.get('candidate_pool_frozen') is True and Path(r.get('pool','')).name=='scored_candidates_v2_fixed_v3_expanded.json' and Path(d.get('scored_candidates','')).name=='scored_candidates_v2_fixed_v3_expanded.json','leakage':'NOT_VERIFIABLE','leakage_manual_paths':['OOF held-out fold manifests','feature-bank source manifests','real train/val provenance']}
 checks=[]
