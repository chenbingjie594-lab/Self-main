...
class MVTecDataset(Dataset):
    ...
    def __init__(
        ...
        mask_by_stem = {
            path.stem: path
            for path in self.mask_data_root.iterdir()
            if path.is_file() and path.suffix.lower() in image_extensions
        }
        paired_stems = sorted(instance_by_stem.keys() & mask_by_stem.keys())
        if not paired_stems:
            raise ValueError(
                "No image/mask pairs with matching filename stems were found. "
                f"images={self.instance_data_root} ({len(instance_by_stem)} files), "
                f"masks={self.mask_data_root} ({len(mask_by_stem)} files)."
            )
        missing_masks = sorted(instance_by_stem.keys() - mask_by_stem.keys())
        missing_images = sorted(mask_by_stem.keys() - instance_by_stem.keys())
        if missing_masks or missing_images:
            raise ValueError(
                "Image/mask filenames are not paired one-to-one: "
                f"missing_masks={len(missing_masks)}, "
                f"missing_images={len(missing_images)}."
            )
        self.instance_images_path = [instance_by_stem[stem] for stem in paired_stems]
        self.mask_images_path = [mask_by_stem[stem] for stem in paired_stems]
